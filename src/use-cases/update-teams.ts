...
export class UpdateTeamsUseCase {
  ...
  async execute(teamsId: string, data: UpdateTeamsUseCaseRequest): Promise<UpdateTeamsUseCaseResponse> {
    ...
    if (typeof data.active === "boolean" && data.active !== teamById.active && data.active !== undefined && data.active !== null) {
