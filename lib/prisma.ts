      ...
      async $allOperations({ operation, model, args, query }) {
        ...
        if (['update', 'delete'].includes(operation) && args.where) {
          try {
            ...
            oldData = await (prisma as any)[model?.toLowerCase() || ''].findFirst({
              where: args.where,
            });
          } catch (error) {
        ...
        try {
          ...
          await prisma.auditLog.create({
            data: {
              ...
              record_id: recordId,
              action,
              old_data: oldData ? oldData : undefined,
              new_data: ['create', 'update', 'upsert'].includes(operation) ? result : undefined,
              changed_by: userId,
            },
          });
        } catch (error) {
